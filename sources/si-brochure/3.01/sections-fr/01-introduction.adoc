...
depuis la création du SI, a été approuvée par la <<CGPM_MET_26>>{nbsp}: elle est
...
sont décrits dans le texte «{nbsp}<<le_bipm_et_la_cdm,Convention du Mètre>>{nbsp}» (page <<le_bipm_et_la_cdm,pagenumber%>>).
