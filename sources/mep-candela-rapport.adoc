...
This 2^nd^ edition has been updated to include, in the definitions of photometric quantities and units, the spectral luminous efficiency functions for mesopic vision based on CIE 191:2010 (CIE, 2010), as well as the 10° spectral luminous efficiency functions based on CIE 165:2005 (CIE, 2005), and to introduce the defining constant for photometry, denoted stem:[ii(K)_{"cd"}], adopted in the International System of Units (SI) 2019 (BIPM, 2019).
...
where stem:[ii(Q)_v] is the luminous energy emitted, transferred or received and t is time
...
[stem%unnumbered]
++++
Phi_v = ii(K)_m int_0^{oo} Phi_{e,lambda} (lambda) V(lambda) "d" lambda
++++


where stem:[ii(K)_m] is maximum luminous efficacy, stem:[Phi_{e,lambda}] is spectral radiant flux, stem:[V(lambda)] is spectral luminous efficiency and stem:[lambda] is wavelength.
====
...
[stem%unnumbered]
++++
Phi_v = int int_{ii Omega} ii(I)_v (vartheta,varphi) sin vartheta "d" varphi "d" vartheta
++++
...
[stem%unnumbered]
++++
ii(I)_v = ii(K)_m int_0^{oo} ii(I)_{e,lambda} (lambda) V(lambda) "d" lambda
++++

where stem:[ii(K)_m] is maximum luminous efficacy, stem:[ii(I)_{e,lambda}] is the spectral radiant intensity at wavelength stem:[lambda], and stem:[V(lambda)] is spectral luminous efficiency.
====
...
[stem%unnumbered]
++++
ii(L)_v = {"d" ii(I)_v} / {"d" A} 1 / {cos alpha}
++++

where stem:[ii(I)_v] is luminous intensity, stem:[ii(A)] is area and stem:[alpha] is the angle between the normal to the surface at the specified point and the given direction

[NOTE]
====
In a practical sense, the definition of luminance can be thought of as dividing a real or imaginary surface into an infinite number of infinitesimally small surfaces, which can be considered as point sources, each of which has a specific luminous intensity, stem:[ii(I)_v], in the specified direction. The luminance of the surface is then the integral of these luminance elements over the whole surface.

The equation in the definition can mathematically be interpreted as a derivative (i.e. a rate of change of luminous intensity with projected area) and could alternatively be rewritten in terms of the average luminous intensity stem:[bar ii(I)_v] as

[stem%unnumbered]
++++
ii(L)_v = lim_{A -> 0} {bar ii(I)_v} / A  1 / {cos alpha}
++++

Hence, luminance is often considered as a quotient of averaged quantities; the area, stem:[ii(A)], should be small enough that uncertainties due to variations in luminous intensity within that area are negligible, otherwise, the quotient stem:[bar ii(L)_v = {bar ii(I)_v}/{A} 1/{cos alpha}] gives the average luminance and the specific measurement conditions must be reported with the result.
====

[NOTE]
====
For a surface being irradiated, an equivalent formula in terms of illuminance, stem:[ii(E)_v], and solid angle, stem:[ii Omega], is stem:[ii(L)_v = {"d" ii(E)_v}/{"d" ii Omega} 1 / {cos theta}] where stem:[theta] is the angle between the normal to the surface being irradiated and the direction of irradiation. This form is useful when the source has no surface (e.g. the sky, the plasma of a discharge).
====

[NOTE]
====
An equivalent formula is stem:[ii(L)_v = {"d" Phi_v}/{"d" G}] where stem:[Phi_v] is luminous flux and stem:[ii(G)] is geometric extent.
====
...
[stem%unnumbered]
++++
Phi_v = int int ii(L)_v cos alpha "d" A "d" ii Omega
++++
====

[NOTE]
====
Since the optical extent, expressed by stem:[G cdot n^2], where stem:[ii(G)] is geometric extent and stem:[n] is refractive index, is invariant, the quantity expressed by stem:[ii(L)_v cdot n^{-2}] is also invariant along the path of the beam if the losses by absorption, reflection and diffusion are taken as 0. That quantity is called "`basic luminance`".
====
...
[stem%unnumbered]
++++
ii(L)_v (x,y,vartheta,varphi) = {partial Phi_v (x,y,vartheta,varphi)} / {partial A(x,y) cdot cos alpha cdot partial ii Omega (vartheta,varphi)}
++++
...
where stem:[Phi_v] is luminous flux and stem:[ii(A)] is the area on which the luminous flux is incident
...
[stem%unnumbered]
++++
ii(E)_v = ii(K)_m int_0^{oo} ii(E)_{e,lambda} (lambda) V(lambda) "d" lambda
++++

where stem:[ii(K)_m] is maximum luminous efficacy, stem:[ii(E)_{e,lambda}(lambda)] is the spectral irradiance at wavelength stem:[lambda], and stem:[V(lambda)] is spectral luminous efficiency.
====
...
The definition of stem:[ii(K)_{"cd"}] relates the unit of luminous flux (lumen, lm) to the unit of radiant flux (watt, W) at wavelength stem:[lambda_{"cd"}]. For wavelengths other than stem:[lambda_{"cd"}], the luminous efficacy is proportional to one of the spectral luminous efficiency functions described in <<cls-4>>.
...
The spectral luminous efficiency function for mesopic vision is denoted by stem:[ii(V)_{"mes";m} (lambda)], and is defined as

[[eq-1]]
[stem]
++++
ii(V)_{"mes";m} (lambda) = 1 / {ii(M)(m)} {m V (lambda) + (1 - m) V'(lambda)} "&#xA0;" "for" "&#xA0;" 0 <= m <= 1
++++
...
stem:[ii(M)(m)]:: is a normalizing function such that stem:[ii(V)_{"mes";m} (lambda)] attains a maximum value of 1.


<<fig-1>> shows the curves of the mesopic spectral luminous efficiency function stem:[ii(V)_{"mes";m} (lambda)] at stem:[m = 0.2, 0.4, 0.6, 0.8] as examples, plotted with stem:[V(lambda)] and stem:[V'(lambda)]. <<table-3>> shows the values of stem:[ii(V)_{"mes";m} (lambda)] at stem:[m = 0.8] as an example, which corresponds to the visual adaptation condition for a typical road lighting luminance level (stem:[~~ 1 "&#xA0;" "cd m"^{-2}]).


[[fig-1]]
.The spectral luminous efficiency functions for mesopic vision, stem:[ii(V)_{"mes";m} (lambda)], at stem:[m = 0.2; 0.4; 0.6; 0.8] as examples, plotted with stem:[V(lambda)] and stem:[V'(lambda)]
...
The spectral luminous efficiency function for 10° photopic vision is denoted by stem:[ii(V)_{10} (lambda)]; its values are given in <<table-5>>, adopted from (ISO/CIE, 2005).
...
* (photopic) luminous intensity, stem:[ii(I)_v]

* (photopic) illuminance, stem:[ii(E)_v]

* (photopic) luminance, stem:[ii(L)_v]
...
* scotopic luminous intensity, stem:[ii(I)_v']

* scotopic illuminance, stem:[ii(E)_v']

* scotopic luminance, stem:[ii(L)_v']


=== Photometric quantities for mesopic vision

The following quantity names and symbols apply for mesopic vision (CIE, 2016b) (using the stem:[ii(V)_{"mes";m} (lambda)] function):
...
The following quantity names and symbols apply for photopic vision using the stem:[ii(V)_{10} (lambda)] function.
...
For a chosen spectral luminous efficiency function stem:[ii(V)_X (lambda)], the relationship between a photometric quantity, such as luminous flux, stem:[Phi_{v,X}], and the corresponding radiometric quantity spectral radiant flux, stem:[Phi_{e,lambda} (lambda)], is given by:

[[eq-2]]
[stem]
++++
Phi_{v,X} = {ii(K)_{"cd"}} / {ii(V)_X (lambda_{"cd"})} int_{lambda} Phi_{e,lambda} (lambda) ii(V)_X (lambda) "d" lambda
++++

where

stem:[ii(K)_{"cd"}]:: is the luminous efficacy of monochromatic radiation of frequency stem:[540 xx 10^12 "&#xA0;" "Hz"] (stem:[683 "&#xA0;" "lm" cdot "W"^{-1}]);

stem:[ii(V)_X (lambda)]:: represents one of the spectral luminous efficiency functions;

stem:[Phi_{v,X}]:: is the luminous flux evaluated using a defined spectral luminous efficiency function stem:[ii(V)_X (lambda)];
...
[stem]
++++
Phi_v = ii(K)_m int_{lambda} Phi_{e,lambda} V (lambda) "d" lambda
++++
...
stem:[ii(K)_m]:: is the maximum luminous efficacy for photopic vision, i.e. the luminous efficacy at the peak of the stem:[V(lambda)] function, which is at a wavelength of stem:[lambda_m = 555 "&#xA0;" "nm"] (exactly), and is given by:
+
--
[stem]
++++
ii(K)_m = ii(K)_{"cd"} cdot [V(lambda_m) // V(lambda_{"cd"})]
++++
...
[stem]
++++
Phi' = ii(K)_m' int_{lambda} Phi_{e,lambda} (lambda) V'_{lambda} "d" lambda
++++
...
[stem]
++++
K'_m = ii(K)_{"cd"} cdot [V'(lambda'_m) // V'(lambda_{"cd"})]
++++
...
[stem]
++++
Phi_{"mes";m} = {ii(K)_{"cd"}} / {ii(V)_{"mes";m} (lambda_{"cd"})} int_{lambda} Phi_{e,lambda} (lambda) ii(V)_{"mes";m} (lambda) "d" lambda
++++
...
stem:[ii(V)_{"mes";m} (lambda)]:: is the spectral luminous efficiency function for mesopic vision (see <<cls-4-3>>)
...
[stem]
++++
ii(K)_{"m,mes";m} = {ii(K)_{"cd"}} / {ii(V)_{"mes";m} (lambda_{"cd"})}
++++

The value of stem:[m], stem:[ii(K)_{"m,mes";m}] varies from stem:[683 "&#xA0;" "lm" cdot "W"^{-1}] at stem:[m = 1] (photopic) to stem:[1700 "&#xA0;" "lm" cdot "W"^{-1}] at stem:[m = 0] (scotopic)
...
[stem]
++++
Phi_{"mes";m} = ii(K)_{"m,mes";m} int_{lambda} Phi_{e,lambda} (lambda) ii(V)_{"mes";m} (lambda) "d" lambda
++++

The values of stem:[m], stem:[ii(K)_{"m,mes";m}] are given in <<table-4>> for representative values of stem:[m].

The value of stem:[m] is determined from the photopic luminance, stem:[ii(L)_{v,"adapt"}], and scotopic luminance, stem:[L'_{v,"adapt"}], of the visual adaptation field footnote:[Determination of adaptation luminance in various application conditions is still in research. An interim recommendation is available in (CIE, 2017).], and it is obtained as the solution for the simultaneous equations (CIE, 2010):

[stem]
++++
ii(L)_{"mes",n} = { m_{(n - 1)} ii(L)_{v,"adapt"} + (1 - m_{(n-1)}) L'_{v,"adapt"} V'(lambda_m) } / {m_{(n-1)} + (1 - m_{(n-1)}) V'(lambda_m)}
++++
...
Mesopic photometric quantities should always be given with the value of stem:[m], e.g. stem:[ii(L)_{"mes";0.4} = 0.12 "&#xA0;" "cd m"^{-2}].
...
[stem]
++++
Phi_10 = ii(K)_{"m",10} int_{lambda} Phi_{e,lambda} (lambda) ii(V)_10 (lambda) "d" lambda
++++
...
is the maximum luminous efficacy for 10° photopic vision, i.e. the luminous efficacy at the peak of the stem:[ii(V)_{10} (lambda)] function, which is at a wavelength of stem:[m], stem:[lambda_10 = 557 "&#xA0;" "nm"] (exactly):

[stem]
++++
ii(K)_{m,10} = ii(K)_{"cd"} cdot [ii(V)_10 (lambda_{m,10}) // ii(V)_10 (lambda_{"cd"})]
++++

(stem:[= 683.601 "&#xA0;" "lm" cdot "W"^{-1} ~~ 684 "&#xA0;" "lm" cdot "W"^{-1}] in standard air).
--

stem:[ii(V)_{10} (lambda)]:: is the spectral luminous efficiency function for 10° photopic vision (see <<cls-4-4>>)
...
NOTE: The stem:[bar y (lambda)] function is identical to the stem:[V(lambda)] function given in <<table-1>>, and stem:[bar y_10 (lambda)] is identical to the stem:[ii(V)_{10} (lambda)] function given in <<table-5>> (CIE, 2005).
...
The tristimulus values stem:[ii(X)_10], stem:[ii(Y)_10], stem:[ii(Z)_10] for a 10° field of view are calculated similarly using stem:[bar x_10 (lambda)], stem:[bar y_10 (lambda)], stem:[bar z_10 (lambda)].
...
It is also known that the spectral luminous efficiency function for photopic vision, stem:[V(lambda)], underestimates the visual response in the blue region, and an improved function, known as the CIE 1988 modified 2° spectral luminous efficiency function for photopic vision, stem:[ii(V)_M (lambda)], was recommended by the CIE as a supplement to, and not a replacement of, stem:[V(lambda)] (CIE, 1990). The physiologically-based function, known as the cone-fundamental-based spectral luminous efficiency function, stem:[ii(V)_F (lambda)], is based on the latest research (CIE, 2015) and is considered to provide further improvements. The stem:[ii(V)_F (lambda)] or stem:[ii(V)_M (lambda)] functions are not recognized by the CIPM and are used only for research purposes to measure perceived brightness more accurately. Benefits and impacts of the new luminous efficiency functions will be evaluated in the near future.
...
[[table-3]]
.Values of spectral luminous efficiency for mesopic vision, stem:[ii(V)_{"mes";m} (lambda)], at stem:[m = 0.8] (i.e. stem:[ii(V)_{"mes";0.8} (lambda)]) as an example (for other stem:[m] values, use <<eq-1>>)
[cols="^,^,^,^,^,^",options="header"]
|===
| stem:[lambda//"nm"] | stem:[ii(V)_{"mes";m} (lambda)] | stem:[lambda//"nm"] | stem:[ii(V)_{"mes";m} (lambda)] | stem:[lambda//"nm"] | stem:[ii(V)_{"mes";m} (lambda)]
...
[[table-5]]
.Values of spectral luminous efficiency for 10° photopic vision, stem:[ii(V)_10 (lambda)] (stem:[lambda] in standard air)
[cols="^,^,^,^,^,^",options="header"]
|===
| stem:[lambda//"nm"] | stem:[ii(V)_10 (lambda)] | stem:[lambda//"nm"] | stem:[ii(V)_10 (lambda)] | stem:[lambda//"nm"] | stem:[ii(V)_10 (lambda)]
