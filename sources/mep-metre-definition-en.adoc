...
[stem]
++++
Delta ii(varphi) = k cdot 2 Delta z = {2 pi Delta z}/{ii(lambda)//2}
++++
...
[cols="<,^,^,^,^",options="header"]
|===
| Atom <a| Theoretical Radius (nm) <a| Experimental radius (nm) <a| Theoretical strain parameter stem:[ii(beta)] (stem:[10^{-24}] stem:[rm(cm)^{-3}]) <a| Experimental strain parameter stem:[ii(beta)] (stem:[10^{-24}] stem:[rm(cm)^(-3)])
...
. Measurement of step height standard artefacts manufactured from single crystal silicon, where the height range of multiple monoatomic steps currently is limited up to stem:[10 " "rm(nm)] and the uncertainties of the monoatomic step heights are stem:[5 " pm")] under UHV conditions and stem:[15 " pm")] under ambient conditions.
...
| Depends on offset distance (stem:[d]) and change in tilt angle (stem:[ii(theta)]); error stem:[= d" "tan ii(theta)], _e.g._ for stem:[d = 1] stem:[rm(mm)], stem:[ii(theta) = 1] second of arc, error = *5 nm*.
...
stem:[d_{220} = 192.0155714(32)] stem:["pm"] with a standard uncertainty of stem:[0.0000032 xx 10^{-12}] stem:[rm(m)],
