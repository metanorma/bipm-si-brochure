...
:title-part-en: Mise en pratique for the definition of the candela in the SI
...
The purpose of this _mise en pratique_, prepared by the Consultative Committee for Photometry and Radiometry (CCPR) of the International Committee for Weights and Measures (CIPM) and formally adopted by the CIPM, is to indicate how the definition of the SI base unit, the candela, symbol cd, and the derived SI units lumen (stem:["unitsml(lm)"]), lux (stem:["unitsml(lx)"]) and candela per square metre (stem:["unitsml(cd m^{-2})"]) may be realized in practice.
...
For further details of the spectral luminous efficiency functions of the human eye and the definitions of the associated photometric quantities, see _CIPM/CIE joint publication: Principles Governing Photometry, 2nd Edition_ <<photometry>>.
