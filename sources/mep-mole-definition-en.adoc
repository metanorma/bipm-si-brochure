...
[stem]
++++
n = ii(Q) / {z ii(N)_"A" e} = ii(Q) / {zF}
++++


The Faraday constant stem:[ii(F)] has the unit stem:["C/mol"] and is known exactly (stem:[F = ii(N)_"A" e]). The uncertainty of this method of realizing the mole depends on the reaction efficiency of the ion of interest and no interfering ions present.
