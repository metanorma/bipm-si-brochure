...
[stem]
++++
N = (w("X") m) / (m_"a" ("X")) = {w("X")m} / {A_"r" ("X") m_{rm(u)}}
++++
...
[stem]
++++
n = N / N_"A" = {w("X") m} / {A_"r" ("X") N_"A" m_{rm(u)}} = {w("X")m} / {A_"r" ("X") ii(M)_{rm(u)}}
++++
...
[stem]
++++
ii(M)_{rm(u)} = N_"A" m_{rm(u)} = {2 N_"A" h} / c {R_{oo}} / {alpha^2 A_"r" ("e")}
++++
