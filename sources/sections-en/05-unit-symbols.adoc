...
[NOTE]
====
For example: The maximum electric potential difference is stem:[ii(U)_("max") = 1000 "unitsml(V)"]  but not stem:[ii(U) = 1000" "ii(V)_{"max"})]. The mass fraction of copper in the sample of silicon is stem:[w("Cu") = 1.3 xx 10^(-6)] but not stem:[1.3 xx 10^(-6) "unitsml(w)"//"unitsml(w)"].
====
