...
Symbols for quantities are generally single letters set in an italic font, although they may be qualified by further information in subscripts or superscripts or in brackets. For example, stem:[C] is the recommended symbol for heat capacity, stem:[c_m] for molar heat capacity, stem:[c_(m,p)] for molar heat capacity at constant pressure, and stem:[c_(m,V)] for molar heat capacity at constant volume.
...
[NOTE]
====
For example: The maximum electric potential difference is stem:[U_(max) = 1000 text( )V]  but not stem:[U = 1000 text( )V_(max)]. The mass fraction of copper in the sample of silicon is stem:[w(Cu) = 1.3 times 10^(-6)] but not stem:[1.3 times 10^(-6) wtext(/)w].
====
...
[NOTE]
====
stem:[m = 12.3 text( )g] where stem:[m] is used as a symbol for the quantity mass, but stem:[phi = 30° 22text(') 8text(")], where stem:[phi] is used as a symbol for the quantity plane angle.
====

This rule means that the symbol °C for the degree Celsius is preceded by a space when one expresses values of Celsius temperature stem:[t].
...
====
[align=left]
stem:[l = 10.234 text( )m] +
but not +
stem:[l = 10 text( )m text( )23.4 text( )cm]
====
...
NOTE: 43 279.168 29 but not 43,279.168,29

NOTE: either 3279.1683 or 3 279.168 3
...
[stem%unnumbered]
++++
m_n = 1.674 927 471 (21) times 10^(−27) text( )kg,
++++

where stem:[m_n] is the symbol for the quantity (in this case the mass of a neutron) and the number in parentheses is the numerical value of the standard uncertainty of the estimated value of stem:[m_n] referred to the last digits of the quoted value; in this case stem:[u(m_n) = 0.000 000 021 times 10^(−27) text( )kg]. If an expanded uncertainty stem:[U(x)] is used in place of the standard uncertainty stem:[u(x)], then the coverage probability stem:[p] and the coverage factor stem:[k] must be stated.
...
[align=left]
stem:[(53 text( )m text(/)s) times 10.2 text( )s] +
or stem:[(53 text( )m text(/)s)(10.2 text( )s)]

[align=left]
stem:[25 times 60.5] +
but not stem:[25 times 60.5]

stem:[(20 text( )m)text(/)(5 text( )s) = 4 text( )mtext(/)s]

stem:[(atext(/)b)text(/)c], not stem:[atext(/)btext(/)c]

[align=left]
stem:[n = 1.51], +
but not stem:[n = 1.51 times 1], +
where stem:[n] is the +
...
Quantities that are ratios of quantities of the same kind (for example length ratios and amount fractions) have the option of being expressed with units (stem:[mtext(/)m], stem:[mol text(/) mol]) to aid the understanding of the quantity being expressed and also allow the use of SI prefixes, if this is desirable (stem:[mum text(/) m], stem:[nmol text(/)mol]). Quantities relating to counting do not have this option, they are just numbers.
...
One radian corresponds to the angle for which stem:[s = r], thus stem:[1 text( )rad = 1]. The measure of the right angle is exactly equal to the number stem:[pi text(/)2].
...
The solid angle, expressed in steradian, corresponds to the ratio between an area stem:[A] of the surface of a sphere of radius stem:[r] and the squared radius, stem:[Omega = Atext(/)r^2 text( )sr]. One steradian corresponds to the solid angle for which stem:[A = r^2], thus stem:[1 text( )sr = 1].
