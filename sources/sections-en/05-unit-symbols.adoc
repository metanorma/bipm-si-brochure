...
Unit names are normally printed in upright type and they are treated like ordinary nouns. In English, the names of units start with a lower-case letter (even when the symbol for the unit begins with a capital letter), except at the beginning of a sentence or in capitalized material such as a title. In keeping with this rule, the correct spelling of the name of the unit with the symbol stem:["unitsml(°C)"] is "degree Celsius" (the unit degree begins with a lower-case d and the modifier Celsius begins with an upper-case C because it is a proper name).
...
[NOTE]
====
For example: The maximum electric potential difference is stem:[ii(U)_("max") = 1000 " ""unitsml(V)"]  but not stem:[ii(U) = 1000 " ""unitsml(ii(V)"_(max))]. The mass fraction of copper in the sample of silicon is stem:[w("Cu") = 1.3 xx 10^(-6)] but not stem:[1.3 xx 10^(-6) " ""unitsml(w)"//"unitsml(w)"].
====
...
[NOTE]
====
stem:["m" = 12.3 " ""unitsml(g)"] where stem:["unitsml(m)"] is used as a symbol for the quantity mass, but stem:[ii(phi) = 30° 22&#x2032; 8&#x2033;], where stem:[ii(phi)] is used as a symbol for the quantity plane angle.
====

This rule means that the symbol stem:["unitsml(°C)"] for the degree Celsius(((degree Celsius (°C)))) is preceded by a space when one expresses values of ((Celsius temperature)) stem:[t].

[NOTE]
====
stem:[t = 30.2 " ""unitsml(°C)"] +
but not +
stem:[t = 30.2 " ""unitsml(°C)"] +
nor stem:[t = 30.2 " ""unitsml(°C)"]
====
...
[NOTE]
====
a stem:[10 " ""unitsml(k Omega)"] resistor +
...
[[plane_angles]]
==== Plane angles, solid angles and phase angles (((radian (rad)))) (((steradian (sr))))
...
A historical convention is the degree. The conversion between radians and degrees follows from the relation stem:[360° = 2pi " ""unitsml(rad)"]. Note that the degree, with the symbol °, is not a unit of the SI.
(((steradian (sr))))
...
The units stem:["unitsml(rad)"] and stem:["unitsml(sr)"] correspond to ratios of two lengths and two squared lengths, respectively. However, it shall be emphasized that rad and sr must only be used to express angles and solid angles, but not to express ratios of lengths and squared lengths in general.
