...
[example]
====
stem:["unitsml(p)""unitsml(m)"] (picometre), stem:["unitsml(mmol)"] (millimole), stem:["unitsml(G)" Omega] (gigaohm), stem:["unitsml(THz)"] (terahertz)
...
Similarly prefix names are also inseparable from the unit names to which they are attached. Thus, for example, millimetre, micropascal and meganewton are single words. (((pascal (Pa))))
