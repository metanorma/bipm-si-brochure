...
[lowerroman]
. the hertz, joule, coulomb, lumen, and watt, with unit symbols stem:[rm(Hz), " "rm(J), " "rm(C), " "rm(lm)], and stem:[rm(W)], respectively, are related to the units second, metre, kilogram, ampere(((ampere (A)))), kelvin(((kelvin (K)))), mole, and candela(((candela (cd)))), with unit symbols stem:[rm(s), " "rm(m), " "rm(kg), " "rm(A), " "rm(K), " "rm(mol)], and stem:[rm(cd)], respectively, according to stem:[rm(Hz) = rm(s)^(−1)], stem:[rm(J) = rm(m)^2 " "rm(kg)" "rm(s)^(−2)], stem:[rm(C) = rm(s)" "rm(A)], stem:[rm(lm) = rm(cd)" "rm(m)^2 " "rm(m)^(−2) = rm(cd)" "rm(sr)], and stem:[rm(W) = rm(m)^2 " "rm(kg)" "rm(s)^(−3)],
...
*further notes* that since (((kelvin (K))))(((mole (mol))))
